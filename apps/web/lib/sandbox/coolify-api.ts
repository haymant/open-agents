...
function fsApiUrlWithoutPort(url: string): string {
  try {
    const p = new URL(url);
    if (p.port === "1223" || p.port === "1222") p.port = "";
...
async function fsApiJsonRequest<T = unknown>(
  url: string,
  ...
  try {
    ...
    if (!(err instanceof Error) || !isSelfSignedTlsError(err)) {
      throw new CoolifyApiError(
        `FS API request to ${url} failed: ${err instanceof Error ? err.message : String(err)}`,
        502,
      );
    }
    const text = await fsApiRequestInsecure(url, init);
  ...
  if (!resp.ok) {
    ...
    throw new CoolifyApiError(
      `FS API returned ${resp.status}: ${text || resp.statusText}`,
      resp.status,
    );
  }
...
export async function snapshotCoolifyWorkspace(
  fsApiBaseUrl: string,
  apiToken?: string,
): Promise<string> {
  ...
  const baseUrl = fsApiUrlWithoutPort(fsApiBaseUrl);
  const SNAPSHOT_FILE = ".snapshot/workspace.tar.gz";

  const execR = await fsApiJsonRequest<{
    success: boolean;
    code: number | null;
    stderr: string;
    stdout: string;
    ...
    headers,
    body: JSON.stringify({
      cmd: `mkdir -p /workspace/.snapshot && tar czf /workspace/${SNAPSHOT_FILE} --exclude=.snapshot --exclude=node_modules --exclude=.git -C /workspace .`,
      cwd: "/workspace",
      timeoutMs: 60_000,
    }),
  });
  ...
  if (!execR.success) {
    throw new CoolifyApiError(
      `Failed to create workspace snapshot: ${execR.stderr || execR.stdout || `exit code ${execR.code}`}`,
      502,
    );
  }

  const readR = await fsApiJsonRequest<{ content: string }>(
    `${baseUrl}/api/files/content?encoding=base64&path=.snapshot/workspace.tar.gz`,
    { headers },
  );

  // best-effort cleanup
  fsApiJsonRequest(`${baseUrl}/api/exec`, {
    method: "POST",
    headers,
    body: JSON.stringify({
      cmd: "rm -rf /workspace/.snapshot",
      cwd: "/workspace",
      timeoutMs: 10_000,
    }),
  }).catch(() => undefined);

  return readR.content;
}

export async function restoreCoolifyWorkspace(
  fsApiBaseUrl: string,
  snapshotData: string,
  apiToken?: string,
): Promise<void> {
  ...
  for (let attempt = 0; attempt < 10; attempt++) {
    ...
    try {
      await doRestore(baseUrl, headers, snapshotData);
      return;
    } catch (err) {
      lastError = err instanceof Error ? err : new Error(String(err));
      console.log(
        `[restoreCoolifyWorkspace] attempt ${attempt + 1}/10 failed: ${lastError.message}`,
      );
    }
  }
  throw lastError ?? new Error("Failed to restore workspace snapshot");
}
...
async function doRestore(
  ...
  const RESTORE_FILE = ".snapshot/workspace.tar.gz";

  await fsApiJsonRequest(`${baseUrl}/api/files/content`, {
    method: "POST",
    headers,
    body: JSON.stringify({
      path: RESTORE_FILE,
  ...
  const extractR = await fsApiJsonRequest<{
    success: boolean;
    code: number | null;
    stderr: string;
    stdout: string;
    ...
    headers,
    body: JSON.stringify({
      cmd: `tar xzf /workspace/${RESTORE_FILE} -C /workspace && rm -rf /workspace/.snapshot`,
      cwd: "/workspace",
      timeoutMs: 60_000,
    }),
  });
  ...
  if (!extractR.success) {
    throw new CoolifyApiError(
      `Failed to restore workspace snapshot: ${extractR.stderr || extractR.stdout || `exit code ${extractR.code}`}`,
      502,
    );
  }
}
